    ...
    <>
      ...
      <Tabs
        screenOptions={{
          ...
          headerRight: () => (
            <TouchableOpacity onPress={toggleTheme} style={[styles.themeButton, { paddingRight: Math.max(insets.right, Spacing.five) }]} activeOpacity={0.7}>
              <FontAwesome6 name={theme === 'light' ? 'moon' : 'sun'} size={24} color={colors.text} />
            </TouchableOpacity>
          ),
          headerStyle: { backgroundColor: colors.background, elevation: 0, shadowOpacity: 0, height: Platform.OS === 'ios' ? 110 : 90 },
          tabBarStyle: { backgroundColor: colors.background, borderTopColor: colors.border, borderTopWidth: 0.5, paddingBottom: isIOS ? insets.bottom : Spacing.two, paddingTop: Platform.OS === 'ios' ? Spacing.two : Spacing.one, height: Platform.OS === 'ios' ? 85 : 65 },
...
const styles = StyleSheet.create({
  headerLeft: { flexDirection: 'column', justifyContent: 'center', paddingVertical: Spacing.three },
  headerTitle: { flexDirection: 'row', alignItems: 'center', gap: Spacing.two },
  logo: { fontSize: Platform.OS === 'ios' ? 28 : 26, fontWeight: '800', letterSpacing: -0.5 },
  subhead: { fontSize: Platform.OS === 'ios' ? 12 : 11, marginTop: Spacing.one },
  themeButton: { padding: Spacing.two },
  tabLabel: { fontSize: Platform.OS === 'ios' ? 10 : 11, fontWeight: '500', marginBottom: Platform.OS === 'ios' ? 0 : Spacing.one, marginTop: Platform.OS === 'ios' ? 2 : 0 },
});